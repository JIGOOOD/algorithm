...
**모든 출입구를 동시에 시작점**으로 만드는 다익스트라를 적용한다. 그러면 탐색은 하나의 간선만 하기 때문에 시간 복잡도는 O(ElogV)가 된다.

### ✏️ 알고리즘 풀이

이건 최소 경로를 구하는 게 아니라, 경로 상 최대 intensity를 구해야 한다.

이때 intensity란, 그냥 weight와 같다.

그러므로 현재 가려는 곳의 intensity가 현재까지의 intensity(cw)와 본인이 가지고 있는 intensity(nw)의 max 값보다 작은 곳으로 가야 한다.

또한, 봉우리는 한번만 거쳐야 하며, 출발지는 거치면 안된다.

다익스트라를 거친 후, 마지막에는 intensity가 작은 값을 구해야 한다.

## 💭 오늘의 회고

처음 알게 된 유용한 사실: **set은 in 시간 복잡도가 O(1)이다.**
