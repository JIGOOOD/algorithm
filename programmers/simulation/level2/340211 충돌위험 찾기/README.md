...
규칙(1~5)에 따르면 얘네의 최단 거리는 하나로 정해진다. 그러므로 그냥 시뮬레이션 하면 된다.

### 이동

    현재 x값이 타겟 값보다 작으면 +1 크면 -1 하는 식으로 이동한다. y도 동일하게 이동한다.

    격자 크기가 최대 100 * 100이므로, 한번의 이동 당 최대 이동 횟수는 200이다.

    그럼 로봇 1대 당 이동 횟수는 200 * 100 = 2,000이고,

    총 이동 횟수는 200 * 100 * 100 = 2,000,000 = 200만이다.

### 시간

    총 가능한 시간은 200 * 100 = 20,000이다.

    dp[t][x][y] = t 시간에 x, y좌표에 도달할 경우의 수를 구한다면, 20,000 * 100 * 100 = 2억이 되므로, 3차원 배열에 해당 값을 저장할 수는 없다.

    아까 <이동>란에서 계산했던 것 처럼 총 이동 횟수는 200만 이므로, 굳이 3차원 배열을 쓰지 않아도 된다.

    그러므로 딕셔너리를 활용해 값을 저장해준다.

    즉, 이동할 때마다 튜플 (t, x, y)를 key로 하는 딕셔너리에 +1 씩 해준다.

### 계산

    이후 딕셔너리의 값을 돌면서 값이 2 이상인 애들만 count 해주면, 정답이 도출된다.
